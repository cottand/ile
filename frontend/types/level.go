...
func (t *Fresher) freshenAboveWithRigidify(l level, limit level, type_ SimpleType, rigidify bool) SimpleType {
	return t.freshen(l, limit, type_, rigidify, make(map[typeVariableID]SimpleType))
}

func (t *Fresher) freshen(l level, limit level, type_ SimpleType, rigidify bool, freshened map[typeVariableID]SimpleType) SimpleType {
	...
	case *typeVariable:
		...
		if !rigidify {
			...
			for _, lb := range type_.lowerBounds {
				freshV.lowerBounds = append(freshV.lowerBounds, t.freshen(l, limit, lb, rigidify, freshened))
			}
			for _, ub := range type_.upperBounds {
				freshV.upperBounds = append(freshV.upperBounds, t.freshen(l, limit, ub, rigidify, freshened))
			}
			return freshV
		}
		...
		for _, lb := range type_.lowerBounds {
			freshLb := t.freshen(l, limit, lb, rigidify, freshened)
		...
		for _, up := range type_.upperBounds {
			freshUp := t.freshen(l, limit, up, rigidify, freshened)
	...
	case typeRange:
		if !rigidify {
			return typeRange{
				lowerBound:     t.freshen(l, limit, type_.lowerBound, rigidify, freshened),
				upperBound:     t.freshen(l, limit, type_.upperBound, rigidify, freshened),
				withProvenance: type_.withProvenance,
			}
		}
		freshVar := t.newTypeVariable(l, type_.provenance, "", nil, nil)
		freshVar.lowerBounds = append(freshVar.lowerBounds, t.freshen(l, limit, type_.lowerBound, rigidify, freshened))
		freshVar.upperBounds = append(freshVar.upperBounds, t.freshen(l, limit, type_.upperBound, rigidify, freshened))
		return freshVar
	case funcType:
		var fields = make([]SimpleType, len(type_.args))
		for i, field := range type_.args {
			fields[i] = t.freshen(l, limit, field, rigidify, freshened)
		}
		return funcType{
			args: fields,
			ret:  t.freshen(l, limit, type_.ret, rigidify, freshened),
		}
	case unionType:
		return unionType{
			lhs: t.freshen(l, limit, type_.lhs, rigidify, freshened),
			rhs: t.freshen(l, limit, type_.rhs, rigidify, freshened),
		}
	...
	case negType:
		return negType{
			negated: t.freshen(l, limit, type_.negated, rigidify, freshened),
		}
	case extremeType:
		return type_
	case tupleType:
	...
	case namedTupleType:
		...
		for i, field := range type_.fields {
			fields[i] = util.Pair[ast.Var, SimpleType]{
				Fst: field.Fst,
				Snd: t.freshen(l, limit, field.Snd, rigidify, freshened),
			}
		}
	...
	case arrayType:
		return arrayType{
			innerT:         t.freshen(l, limit, type_.innerT, rigidify, freshened),
			withProvenance: type_.withProvenance,
		}
	...
	case typeRef:
		...
		for i, typeArg := range type_.typeArgs {
			typeArgs[i] = t.freshen(l, limit, typeArg, rigidify, freshened)
		}
		return typeRef{
