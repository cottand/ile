...
func (infer *inferer) consGen(expr Expression) (err error) {
	...
	case Apply:
		...
		if err = infer.consGen(et.Body()); err != nil {
			return errors.Wrapf(err, "Unable to infer body of Apply: %v. BodyLit: %v", et, et.Body())
		}
	...
	case LetRec:
		...
		if err = infer.consGen(et.Body()); err != nil {
			return errors.Wrapf(err, "Unable to infer body of letRec %v. BodyLit: %v", et, et.Body())
		}
	...
	case Let:
		...
		if err = infer.consGen(et.Body()); err != nil {
			return errors.Wrapf(err, "Unable to infer body of let %v. BodyLit: %v", et, et.Body())
		}
